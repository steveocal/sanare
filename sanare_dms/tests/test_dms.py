...
class TestSanareDms(TransactionCase):
    ...
    def test_email_block_send(self):
        P = self.env["res.partner"]
        a = P.create({"name": "Alice Adams", "email": "alice@example.com"})
        b = P.create({"name": "Bob Brown", "email": "bob@example.com"})
        c = P.create({"name": "Carol Clark", "email": "carol@example.com"})
        d = P.create({"name": "Dan Dole", "email": "dan@example.com"})
        doc = self.Doc.create({
            "name": "Email Tpl", "content_type": "knowledge_html",
            "content_html": self._EMAIL_MARKER + "<p>Dear {{recipient_first_name}},</p>",
        })
        res = doc.send_email_block(doc.id, {
            "subject": "Hi {{recipient_first_name}}",
            "to_ids": [a.id, b.id], "cc_ids": [c.id], "bcc_ids": [d.id],
            "body_html": "<p>Dear {{recipient_first_name}}, from {{sender_name}}</p>",
        })
        self.assertIn(res["state"], ("sent", "failed"))
        self.assertTrue(res["mail_id"])

        # primary (To + Cc) + one blind copy per Bcc partner, same subject
        mails = self.env["mail.mail"].search([("subject", "=", "Hi Alice")])
        self.assertEqual(len(mails), 2)
        primary = self.env["mail.mail"].browse(res["mail_id"])
        self.assertIn("alice@example.com", primary.email_to)
        self.assertIn("bob@example.com", primary.email_to)
        self.assertIn("carol@example.com", primary.email_cc or "")
        # placeholders resolved in both subject and body
        self.assertNotIn("{{", primary.subject)
        self.assertIn("Alice", primary.subject)
        self.assertNotIn("{{recipient_first_name}}", primary.body_html)
        self.assertNotIn("{{sender_name}}", primary.body_html)
        bcc_mail = mails - primary
        self.assertIn("dan@example.com", bcc_mail.email_to)
        self.assertFalse(bcc_mail.email_cc)
        # audit line posted to the document chatter
        self.assertTrue(
            doc.message_ids.filtered(lambda m: "sent to" in (m.body or ""))
        )

    def test_email_block_requires_marker(self):
        doc = self.Doc.create({
        ...
        with self.assertRaises(UserError):
            doc.send_email_block(doc.id, {
                "subject": "x", "to_ids": [p.id], "cc_ids": [], "bcc_ids": [],
                "body_html": "<p>x</p>",
            })

    def test_email_block_missing_email(self):
        noemail = self.env["res.partner"].create({"name": "No Email Person"})
        doc = self.Doc.create({
            "name": "Email Tpl 2", "content_type": "html",
            "content_html": self._EMAIL_MARKER + "<p>body</p>",
        })
        before = self.env["mail.mail"].search([])
        res = doc.send_email_block(doc.id, {
            "subject": "Hello", "to_ids": [noemail.id], "cc_ids": [], "bcc_ids": [],
            "body_html": "<p>x</p>",
        })
    ...
    def test_email_block_stripped_from_server_render(self):
        doc = self.Doc.create({
            "name": "Email Tpl 3", "content_type": "html",
            "content_html": self._EMAIL_MARKER + "<p>visible-body</p>",
        })
        rendered = doc._resolve_embedded_refs(doc.content_html)
        self.assertNotIn("sanareEmailSend", rendered)
        self.assertIn("visible-body", rendered)
