...
class EosSku(models.Model):
    ...
    inventory_cm2 = fields.Float(
        string='Inventory cm2', compute='_compute_inventory', store=True,
        help='On-hand quantity read from Odoo Inventory (stock.quant).')
    inventory_value = fields.Monetary(
        string='Inventory Value', compute='_compute_inventory', store=True,
    ...
    def _compute_inventory(self):
        for sku in self:
            qty = sku.product_id.qty_available if sku.product_id else 0.0
            price = sku.product_id.standard_price if sku.product_id else 0.0
            sku.inventory_cm2 = qty
            sku.inventory_value = qty * price
